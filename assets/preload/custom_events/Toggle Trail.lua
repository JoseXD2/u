...
function createTrailFrame(tag)
	...
	if num - trailLength + 1 >= 0 then
		for i = (num - trailLength + 1), (num - 1) do
			setProperty('crossFade'..tag..i..'.alpha', getProperty('crossFade'..tag..i..'.alpha') - (0.6 / (trailLength - 1)));
		end
	end
	removeLuaSprite('crossFade'..tag..(num - trailLength));

	if not (image == '') then
		trailTag = 'crossFade'..tag..num;
